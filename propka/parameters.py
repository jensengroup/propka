...
STRING_LISTS = [
    ...
    "acid_list",
    "base_list",
    "exclude_sidechain_interactions",
    "backbone_reorganisation_list",
    "write_out_order",
]
...
PARAMETERS = [
    "Nmin",
    "Nmax",
    "desolvationSurfaceScalingFactor",
    "desolvationPrefactor",
    "desolvationAllowance",
    "coulomb_diel",
    "COO_HIS_exception",
    "OCO_HIS_exception",
    "CYS_HIS_exception",
    "CYS_CYS_exception",
    "min_ligand_model_pka",
    "max_ligand_model_pka",
    "include_H_in_interactions",
    "coupling_max_number_of_bonds",
    "min_bond_distance_for_hydrogen_bonds",
    "coupling_penalty",
    "shared_determinants",
    "common_charge_centre",
    "hide_penalised_group",
    "remove_penalised_group",
    "max_intrinsic_pka_diff",
    "min_interaction_energy",
    "max_free_energy_diff",
    "min_swap_pka_shift",
    "min_pka",
    "max_pka",
...
class Parameters:
    ...
    def set_up_data_structures(self):
        ...
        """
        for key_word in (
            NUMBER_DICTIONARIES + LIST_DICTIONARIES + STRING_DICTIONARIES
        ):
    ...
    def print_interaction_parameters(self):
        ...
        for k in self.model_pkas:
            info(f"{k:>3s} {self.model_pkas[k]:8.2f}")
                ...
                if group2 in map_:
                    for val in map_[group2]:
                        ...
                        if (
                            ...
                            map_interaction += "  "
                    if len(map_[group2]) == 0 and (
                        self.sidechain_cutoffs.get_value(group1, group2)[0]
                        != 3
                        or self.sidechain_cutoffs.get_value(group1, group2)[1]
                        != 4
                    ):
                        map_interaction += "?  "
    ...
    def print_interaction_parameters_latex(self):
        ...
        lines = [
            ...
            (
                ...
                "listed, the default value of {0:s} is applied.}}"
            ).format(self.sidechain_cutoffs.default),
    ...
    def print_interactions_latex(self):
        ...
        lines = [
            ...
            (
                "\\caption{{Ligand interaction parameters. For interactions not "
                "listed, the default value of {0:s} is applied.}}"
            ).format(str(self.sidechain_cutoffs.default)),
            "\\label{{tab:ligand_interaction_parameters}}\\\\",
            "\\toprule",
            "Group1 & Group2 & Interaction & c1 &c2 \\\\",
            "\\midrule",
...
class InteractionMatrix:
    ...
    def __getitem__(self, group):
        ...
        if group not in self.dictionary.keys():
            raise KeyError(
                f"{group} not found in interaction matrix {self.name}"
            )
...
class PairwiseMatrix:
    ...
    def insert(self, key1, key2, value):
        ...
        if key1 in self.dictionary and key2 in self.dictionary[key1]:
            if key1 != key2:
                str_ = (
                    f"Parameter value for {key1}, "
                    f"{key2} defined more than once"
                )
                warning(str_)
    ...
    def __str__(self):
        ...
        for key1 in self.keys():
            for key2 in self[key1].keys():
                str_ += f"{key1} {key2} {self[key1][key2]}\n"
        return str_
