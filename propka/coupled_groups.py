...
class NonCovalentlyCoupledGroups:
    ...
    def is_coupled_protonation_state_probability(
        self, group1, group2, energy_method, return_on_fail=True
    ):
        ...
        interaction_energy = max(
            self.get_interaction(group1, group2),
        ...
        if (
            (
                max(default_pka1, default_pka2) < self.parameters.min_pka
        ...
        if (
            abs(default_energy - swapped_energy)
        ...
        # check pka shift
        if (
            max(abs(pka_shift1), abs(pka_shift2))
        ...
        if (
            abs(group1.intrinsic_pka - group2.intrinsic_pka)
    ...
    def get_pka_diff_factor(self, pka1, pka2):
        ...
        if intrinsic_pka_diff <= self.parameters.max_intrinsic_pka_diff:
            res = (
                1
                - (intrinsic_pka_diff / self.parameters.max_intrinsic_pka_diff)
                ** 2
            )
        return res
    ...
    def get_free_energy_diff_factor(self, energy1, energy2):
        ...
        if free_energy_diff <= self.parameters.max_free_energy_diff:
            res = (
                1
                - (free_energy_diff / self.parameters.max_free_energy_diff)
                ** 2
            )
        return res
    ...
    def identify_non_covalently_coupled_groups(
        self, conformation, verbose=True
    ):
        ...
        if verbose:
            ...
            info(
                f"\n"
                f" Warning: When using the -d option, pKa values based on \n"
                f"'swapped' interactions\n"
                f"          will be writting to the output .pka file\n"
                f"\n"
                f"{sep}\n"
                f"\n"
                f" Detecting non-covalently coupled residues\n"
                f"{sep}\n"
                f"   Maximum pKa difference:     "
                f"{self.max_intrinsic_pka_diff:>4.2f} pKa units\n"
                f"   Minimum interaction energy: "
                f"{self.min_interaction_energy:>4.2f} pKa units\n"
                f"   Maximum free energy diff.:  "
                f"{self.max_free_energy_diff:>4.2f} pKa units\n"
                f"   Minimum swap pKa shift:     "
                f"{self.min_swap_pka_shift:>4.2f} pKa units\n"
                f"   pH:                         {self.pH:>6} \n"
                f"   Reference:                  {self.reference}\n"
                f"   Min pKa:                    {self.min_pka:>4.2f}\n"
                f"   Max pKa:                    {self.max_pka:>4.2f}\n\n"
            )
            ...
            for group1 in titratable_groups:
                for group2 in titratable_groups:
                    if group1 == group2:
                        break
                    if (
                        group1 not in group2.non_covalently_coupled_groups
                        and self.do_prot_stat
                    ):
                        data = self.is_coupled_protonation_state_probability(
                            group1,
                            group2,
    ...
    def print_out_swaps(self, conformation):
        ...
        map_ = make_interaction_map(
            f"Non-covalent coupling map for {str(conformation)}",
    ...
    def print_system(self, conformation, system):
        ...
        """
        info(f"System containing {len(system):d} groups:")
        ...
        for combination in combinations:
            ...
            for interaction in combination:
                swap_info += (
                    f" {interaction[0].label} {interaction[1].label}\n"
                )
            # swap...
            for interaction in combination:
    ...
    def tagged_format(tag, str_, labels):
        ...
        str_ = str_.replace("\n", f"\n{tag} ")
        for label in labels:
            str_ = str_.replace(label, f"\033[31m{label}\033[30m")
        return str_ + "\n"
    ...
    def make_data_to_string(data, group1, group2):
        ...
        """
        return (
            f" {group1.label} and {group2.label} coupled (prot.state):"
            f" {data['coupling_factor']:>5.2f}\n"
            f" Energy levels:       {data['default_energy'],:>6.2f},"
            f" {data['swapped_energy']:>6.2f} "
            f" (difference:"
            f" {data['default_energy'] - data['swapped_energy']:>6.2f})"
            f" at pH {data['pH']:>6.2f}\n"
            f" Interaction energy:  {data['interaction_energy']:>6.2f}\n"
            f" Intrinsic pka's:    "
            f" {group1.intrinsic_pka:>6.2f}, {group2.intrinsic_pka:>6.2f} "
            f" (difference:"
            f" {group1.intrinsic_pka - group2.intrinsic_pka:>6.2f})\n"
            f" Swapped pKa's:      "
            f" {data['swapped_pka1']:>6.2f}, {data['swapped_pka2']:>6.2f} "
            f" (difference:"
            f" {data['pka_shift1']:>6.2f}, {data['pka_shift2']:>6.2f})"
        )
