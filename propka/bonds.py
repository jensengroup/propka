...
class BondMaker:
    ...
    def __init__(self):
        ...
        distances = list(self.distances_squared.values()) + [
            self.default_dist_squared
        ]
        self.max_sq_distance = max(distances)
        # protein bonding data
        self.data_file_name = pkg_resources.resource_filename(
            __name__, "protein_bonds.json"
        )
        ...
        self.num_pi_elec_conj_bonds_sidechains = {
            "ARG-NE": 1,
            "ARG-NH2": 1,
            "ASN-ND2": 1,
            "GLN-NE2": 1,
        ...
        self.num_pi_elec_bonds_ligands = {
            "C.ar": 1,
            "N.pl3": 0,
            "C.2": 1,
            "O.2": 1,
    ...
    def find_bonds_for_protein(self, protein):
        ...
        for chain in protein.chains:
            for i in range(1, len(chain.residues)):
                if chain.residues[i - 1].res_name.replace(" ", "") not in [
                    "N+",
                    "C-",
                ] and chain.residues[i].res_name.replace(" ", "") not in [
                    "N+",
                    "C-",
                ]:
                    self.connect_backbone(
                        chain.residues[i - 1], chain.residues[i]
                    )
        ...
        for atom1 in cys1.atoms:
            if atom1.name == "SG":
                for atom2 in cys2.atoms:
                    if atom2.name == "SG":
                        dist = propka.calculations.squared_distance(
                            atom1, atom2
                        )
    ...
    def find_bonds_for_residue_backbone(self, residue):
        ...
        for atom1 in residue.atoms:
            if atom1.name in list(self.num_pi_elec_bonds_backbone.keys()):
                atom1.num_pi_elec_2_3_bonds = self.num_pi_elec_bonds_backbone[
                    atom1.name
                ]
            if atom1.name in (
    ...
    def find_bonds_for_side_chain(self, atoms):
        ...
        """
        for atom1 in atoms:
            key = f"{atom1.res_name}-{atom1.name}"
            ...
            if (
                atom1.name not in self.backbone_atoms
                and atom1.name not in self.terminal_oxygen_names
            ):
                for atom2 in atoms:
                    if atom2.name in (
                        self.protein_bonds[atom1.res_name][atom1.name]
                    ):
                        self.make_bond(atom1, atom2)
        ...
        for atom in atoms:
            ...
            if atom.type == "atom":
                ...
                if atom.name in list(
                    self.num_pi_elec_conj_bonds_backbone.keys()
    ...
    def check_distance(self, atom1, atom2):
        ...
        if sq_dist > self.max_sq_distance:
            return False
        key = f"{atom1.element}-{atom2.element}"
        h_count = key.count("H")
        ...
        if sq_dist < self.default_dist_squared and h_count == 0:
            return True
        return (
            key in self.distances_squared.keys()
            and sq_dist < self.distances_squared[key]
        )
    ...
    def has_bond(atom1, atom2):
        ...
        """
        return atom1 in atom2.bonded_atoms or atom2 in atom1.bonded_atoms
    ...
    def generate_protein_bond_dictionary(self, atoms):
        ...
        for atom in atoms:
            ...
            for bonded_atom in atom.bonded_atoms:
                ...
                if name_i not in (
                    self.backbone_atoms or name_j not in self.backbone_atoms
                ) and name_i not in (
