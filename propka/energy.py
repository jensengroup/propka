...
def calculate_scale_factor(parameters, weight: float) -> float:
    ...
    Returns:
        scaling factor
    """
    scale_factor = (1.0 - (1.0 - parameters.desolvationSurfaceScalingFactor) *
                    (1.0 - weight))
    return scale_factor
...
def calculate_pair_weight(parameters: Parameters, num_volume1: int,
                          num_volume2: int) -> float:
...
def hydrogen_bond_interaction(group1: "Group", group2: "Group", version: "Version"):
    ...
    if closest_atom1 is None or closest_atom2 is None:
        _LOGGER.warning('Side chain interaction failed for {0:s} and {1:s}'.format(
            group1.label, group2.label))
        return None
    # get the parameters
    [dpka_max,
     cutoff] = version.get_hydrogen_bond_parameters(closest_atom1, closest_atom2)
...
def check_coulomb_pair(parameters: Parameters, group1: "Group", group2: "Group",
                       dist: float) -> bool:
...
def check_coo_arg_exception(group_coo, group_arg, version):
    ...
    for _ in ["shortest", "runner-up"]:
        ...
        if closest_arg_atom is None:
            ...
            continue
        [dpka_max,
         cutoff] = version.get_hydrogen_bond_parameters(closest_coo_atom,
                                                        closest_arg_atom)
...
def check_coo_coo_exception(group1, group2, version):
    ...
    [closest_atom1, dist,
     closest_atom2] = get_smallest_distance(interact_groups12, interact_groups21)
    [dpka_max,
     cutoff] = version.get_hydrogen_bond_parameters(closest_atom1, closest_atom2)
    f_angle = 1.00
    value = hydrogen_bond_energy(dist, dpka_max, cutoff, f_angle)
