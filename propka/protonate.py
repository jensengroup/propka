...
class Protonate:
    ...
    def __init__(self, verbose=False):
        ...
        self.sybyl_charges = {'N.pl3': 1, 'N.3': 1, 'N.4': 1, 'N.ar': 1, 'O.co2-': 1}
    ...
    def remove_all_hydrogen_atoms(molecular_container: "MolecularContainer"):
        ...
        for name in molecular_container.conformation_names:
            molecular_container.conformations[name].atoms = (
                molecular_container.conformations[name].get_non_hydrogen_atoms())
    ...
    def set_number_of_protons_to_add(self, atom: Atom):
        ...
        if atom.element not in self.valence_electrons:
            _LOGGER.warning(f'Unknown valence electron for element {atom.element}')
            self.valence_electrons[atom.element] = 4
        atom.number_of_protons_to_add -= self.valence_electrons[atom.element]
        _LOGGER.debug(
            'Valence electrons: {0:>4d}'.format(-self.valence_electrons[atom.element]))
        atom.number_of_protons_to_add -= len(atom.bonded_atoms)
        _LOGGER.debug('Number of bonds:  {0:>4d}'.format(-len(atom.bonded_atoms)))
        atom.number_of_protons_to_add -= atom.num_pi_elec_2_3_bonds
        _LOGGER.debug('Pi electrons:     {0:>4d}'.format(-atom.num_pi_elec_2_3_bonds))
    ...
    def set_steric_number_and_lone_pairs(self, atom: Atom):
        ...
        _LOGGER.debug('{0:>65s}: {1:>4d}'.format('Valence electrons',
                                                 self.valence_electrons[atom.element]))
        atom.steric_number += self.valence_electrons[atom.element]
        _LOGGER.debug('{0:>65s}: {1:>4d}'.format('Number of bonds',
                                                 len(atom.bonded_atoms)))
        atom.steric_number += len(atom.bonded_atoms)
        _LOGGER.debug('{0:>65s}: {1:>4d}'.format('Number of hydrogen atoms to add',
        ...
        _LOGGER.debug('{0:>65s}: {1:>4d}'.format('Number of donated co-ordinated bonds',
                                                 0))
        atom.steric_number += 0
        _LOGGER.debug('{0:>65s}: {1:>4.1f}'.format('Charge(-)', atom.charge))
        atom.steric_number = math.floor((atom.steric_number - atom.charge) / 2)
        atom.number_of_lone_pairs = (atom.steric_number - len(atom.bonded_atoms) -
                                     atom.number_of_protons_to_add)
        _LOGGER.debug('-' * 70)
        _LOGGER.debug('{0:>65s}: {1:>4d}'.format('Steric number', atom.steric_number))
        _LOGGER.debug('{0:>65s}: {1:>4d}'.format('Number of lone pairs',
    ...
    def add_protons(self, atom: Atom):
        ...
        if atom.steric_number in list(self.protonation_methods.keys()):
            ...
            _LOGGER.warning('Do not have a method for protonating %s %s', atom,
    ...
    def trigonal(self, atom: Atom):
        ...
        """
        _LOGGER.debug('TRIGONAL - {0:d} bonded atoms'.format(len(atom.bonded_atoms)))
    ...
    def tetrahedral(self, atom: Atom):
        ...
        Args:
            atom:  atom to protonate.
        """
        _LOGGER.debug('TETRAHEDRAL - {0:d} bonded atoms'.format(len(atom.bonded_atoms)))
        ...
        if len(atom.bonded_atoms) == 2 and atom.number_of_protons_to_add > 0:
            ...
            avec2 = Vector(atom1=atom, atom2=atom.bonded_atoms[1]).rescale(1.0)
            axis = avec1 + avec2
            new_a = rotate_vector_around_an_axis(math.radians(90), axis, -avec1)
            new_a = self.set_bond_distance(new_a, atom.element)
            self.add_proton(atom, cvec + new_a)
        # 3 bonds
    ...
    def set_bond_distance(self, bvec: Vector, element: str) -> Vector:
        ...
        if element in list(self.bond_lengths.keys()):
            ...
            str_ = ('Bond length for {0:s} not found, using the standard value '
