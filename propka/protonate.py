...
class Protonate:
    ...
    def protonate(self, molecules):
        ...
        for name in molecules.conformation_names:
            non_h_atoms = molecules.conformations[
                name
    ...
    def remove_all_hydrogen_atoms(molecular_container):
        ...
        for name in molecular_container.conformation_names:
            molecular_container.conformations[
                name
            ].atoms = molecular_container.conformations[
                name
            ].get_non_hydrogen_atoms()
    ...
    def set_charge(self, atom):
        ...
        if atom.type == "atom":
            key = f"{atom.res_name:3s}-{atom.name:s}"
            if atom.terminal:
                debug(atom.terminal)
                key = atom.terminal
    ...
    def set_number_of_protons_to_add(self, atom):
        ...
        debug(
            f"Valence electrons: {-self.valence_electrons[atom.element]:>4d}"
        )
        atom.number_of_protons_to_add -= len(atom.bonded_atoms)
        debug(f"Number of bonds:  {-len(atom.bonded_atoms):>4d}")
        atom.number_of_protons_to_add -= atom.num_pi_elec_2_3_bonds
        debug(f"Pi electrons:     {-atom.num_pi_elec_2_3_bonds:>4d}")
        atom.number_of_protons_to_add += int(atom.charge)
        debug(f"Charge:           {atom.charge:>4.1f}")
        debug("-" * 10)
    ...
    def set_steric_number_and_lone_pairs(self, atom):
        ...
        debug(
            f"{'Valence electrons':>65s}: "
            f"{self.valence_electrons[atom.element]:>4d}"
        )
        atom.steric_number += self.valence_electrons[atom.element]
        debug(f"{'Number of bonds':>65s}: {len(atom.bonded_atoms):>4d}")
        atom.steric_number += len(atom.bonded_atoms)
        debug(
            f"{'Number of hydrogen atoms to add':>65s}: {atom.number_of_protons_to_add:>4d}"
        )
        atom.steric_number += atom.number_of_protons_to_add
        debug(
            f"{'Number of pi-electrons in double and triple bonds(-)':>65s}: {atom.num_pi_elec_2_3_bonds:>4d}"
        )
        atom.steric_number -= atom.num_pi_elec_2_3_bonds
        debug(
            f"{'Number of pi-electrons in conjugated double and triple bonds(-)':>65s}: {atom.num_pi_elec_conj_2_3_bonds:>4d}"
        )
        atom.steric_number -= atom.num_pi_elec_conj_2_3_bonds
        debug(f"{'Number of donated co-ordinated bonds':>65s}: {0:>4d}")
        atom.steric_number += 0
        debug(f"{'Charge(-)':>65s}: {atom.charge:>4.1f}")
        ...
        debug("-" * 70)
        debug(f"{'Steric number':>65s}: {atom.steric_number:>4d}")
        debug(
            f"{'Number of lone pairs':>65s}: {atom.number_of_lone_pairs:>4d}"
        )
    ...
    def add_protons(self, atom):
        ...
        if atom.steric_number in list(self.protonation_methods.keys()):
            ...
            warning(
                "Do not have a method for protonating",
                atom,
                f"(steric number: {atom.steric_number:d})",
            )
    ...
    def trigonal(self, atom):
        ...
        """
        debug(f"TRIGONAL - {len(atom.bonded_atoms):d} bonded atoms")
    ...
    def tetrahedral(self, atom):
        ...
        Args:
            atom:  atom to protonate.
        """
        debug(f"TETRAHEDRAL - {len(atom.bonded_atoms):d} bonded atoms")
        ...
        if len(atom.bonded_atoms) == 2 and atom.number_of_protons_to_add > 0:
            ...
            avec2 = Vector(atom1=atom, atom2=atom.bonded_atoms[1]).rescale(1.0)
            axis = avec1 + avec2
            new_a = rotate_vector_around_an_axis(
                math.radians(90), axis, -avec1
            )
            new_a = self.set_bond_distance(new_a, atom.element)
            self.add_proton(atom, cvec + new_a)
        # 3 bonds
    ...
    def add_proton(atom, position):
        ...
        new_h.residue_label = (
            f"{new_h.name:<3s}" f"{new_h.res_num:>4d}" f"{new_h.chain_id:>2s}"
        )
        no_protons = atom.count_bonded_elements("H")
        if no_protons > 1:
            for i, proton in enumerate(atom.get_bonded_elements("H"), start=1):
                proton.name = f"H{atom.name[1:]:s}{i:d}"
                proton.residue_label = (
                    f"{proton.name:<3s}"
                    f"{proton.res_num:>4d}"
                    f"{proton.chain_id:>2s}"
                )
    ...
    def set_bond_distance(self, bvec, element):
        ...
        if element in list(self.bond_lengths.keys()):
            ...
            str_ = (
                f"Bond length for {element:s} not found, using the standard value "
                f"of {dist:f}"
            )
            warning(str_)
