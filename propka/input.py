...
def read_molecule_file(
    ...
    """Read a PDB or mmCIF file into a molecular container.
    ...
    Raises:
        ValueError: if the input type or structure data is invalid.
    ...
    Examples:
        ...
        type (based on a `.pdb`, `.cif`, or `.mmcif` extension) and uses
...
def _normalize_altloc(altloc: str) -> str:
    if gemmi.cif.is_null(altloc) or altloc == ' ':
        return 'A'
    if len(altloc) == 1 and altloc in '123456789':
        # Match the existing PDB conversion, e.g. 1 -> A.
        return chr(ord(altloc)+16)
    if len(altloc) == 1 and altloc.isalpha():
        return altloc
    raise ValueError(
        "Invalid mmCIF alternate-location identifier {0!r}; expected a "
        "single letter or digit".format(altloc))


def _normalize_element(element: str) -> str:
    # Normalize element symbols such as c -> C, cl -> Cl, and ZN -> Zn.
