...
class Group:
    def __init__(self, atom):
        #dprint('Made new %s group from %s'%(type,atom))
        self.atom = atom
        self.type = ''
        atom.group = self
...
#            self.label = '%1s%1s%1s%4d%2s'%(self.residue_type[1], atom.element,atom.name[-1], atom.resNumb, atom.chainID)
    ...
    def setup(self):
        ...
        #dprint('ION setup',self,self.residue_type, self.charge)
...
#        dprint('Non-titratable ligand group',atom)
        return
...
class titratable_ligand_group(Group):
    def __init__(self, atom):
        ...
        if atom.marvin_pka:
            ...
            dprint('Titratable ligand group    ',atom, self.model_pka, self.charge)
        self.model_pka_set = True
